class ProfilesAPI {
  ...
  async createProfile(profileName) {
    ...
    await this.localForage.setItem(
      `${this.PROFILE_STORE_NAME}:${profileName}`,
      profile,
    );
    return profile;
  }
  ...
  async getProfile(profileName) {
    return await this.localForage.getItem(
      `${this.PROFILE_STORE_NAME}:${profileName}`,
    );
  }

  async getAllProfiles() {
    const profiles = await this.localForage.keys(
      `${this.PROFILE_STORE_NAME}:*`,
    );
    return Promise.all(
      profiles.map((profileName) => this.getProfile(profileName)),
    );
  }
  ...
  async saveCurrentProfileData() {
    ...
    if (activeProfile) {
      ...
      const idbData = {};
      await Promise.all(
        (await this.localForage.keys(`${this.DATA_STORE_NAME}:*`)).map(
          async (key) => {
      ...
      await this.localForage.setItem(
        `${this.PROFILE_STORE_NAME}:${activeProfile.name}`,
        activeProfile,
      );
    }
  }
