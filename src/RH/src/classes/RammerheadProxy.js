...
class RammerheadProxy extends Proxy {
  ...
  constructor({
    ...
    getServerInfo = (req) => {
      ...
      const { hostname, port } = new URL(`${proto}://${domain}`);
      return {
        hostname,
        port,
        protocol: proto,
  ...
  async _onUpgradeRequest(req, socket, head, serverInfo) {
    ...
    for (const handler of this.onUpgradePipeline) {
      const isRoute = this.checkIsRoute(req);
      if (
        (await handler.call(this, req, socket, head, serverInfo, isRoute)) ===
        true
      ) {
        return;
      }
